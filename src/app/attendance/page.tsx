...
export default function AttendancePage() {
  ...
  const fetchData = async () => {
    ...
    setIsLocked(false);
    try {
      // Ambil semua member (baik yg login maupun manual)
      const { data: memberData } = await supabase.from('profiles').select('id, full_name, no_induk, avatar_url').eq('role', 'member').order('full_name');
  ...
  const handleSubmitAll = async () => {
    ...
    const toastId = toast.loading("Menyimpan & Mengunci...");
    try {
      const upsertPayload = members.map(member => {
        const userData = attendanceData[member.id];
        return { user_id: member.id, date: date, status: userData?.status || 'alpa', notes: userData?.notes || '', activity_name: activityName, is_locked: true };
      });
      // Hapus data lama tanggal tsb (clean slate) lalu insert baru
      await supabase.from('attendance').delete().eq('date', date);
      const { error } = await supabase.from('attendance').insert(upsertPayload);
      if (error) throw error;
      toast.success("Berhasil disimpan!", { id: toastId });
      fetchData();
    } catch (error: any) { toast.error("Gagal: " + error.message, { id: toastId }); } finally { setIsSubmitting(false); }
  };

  // --- LOGIKA GENERATE ID MANUAL (FIXED) ---
  const handleAddMember = async () => {
      if (!newMemberName) { toast.error("Nama wajib diisi"); return; }
      setIsAddingMember(true);
      
      try {
          // 1. Cari nomor urut HANYA dari yang formatnya CM-UNR-...
          // Agar tidak tercampur dengan user login (CM-XXX)
          ...
          if (lastMembers && lastMembers.length > 0) {
              const lastId = lastMembers[0].no_induk; // Contoh: CM-UNR-005
              const parts = lastId.split('-'); // ["CM", "UNR", "005"]
          ...
          // Format: CM-UNR-001 (Padding 3 digit)
          ...
          const { error } = await supabase.from('profiles').insert({
              ...
              no_induk: autoInduk, // Kita paksa pakai ID manual ini
              role: 'member',
              phone_number: null,
              avatar_url: null
          });
  ...
  // Helper Stats
  const getStats = () => { let s = { hadir: 0, izin: 0, sakit: 0, alpa: 0, kosong: 0 }; members.forEach(m => { const st = attendanceData[m.id]?.status; if (st === 'hadir') s.hadir++; else if (st === 'izin') s.izin++; else if (st === 'sakit') s.sakit++; else if (st === 'alpa') s.alpa++; else s.kosong++; }); return s; };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-32 font-sans text-slate-800">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            {/* Tampilkan Badge khusus untuk ID Manual */}
      ...
      {!isLocked ? (
        <div className="fixed bottom-0 left-0 right-0 p-6 bg-white/80 backdrop-blur-md border-t border-slate-100 z-40"><div className="max-w-3xl mx-auto flex items-center justify-between gap-4"><div className="hidden sm:block"><p className="text-xs font-bold text-slate-400">Total Anggota</p><p className="text-xl font-black text-slate-800">{members.length} <span className="text-sm font-medium text-slate-400">Orang</span></p></div><button onClick={handleSubmitAll} disabled={isSubmitting || loading} className="flex-1 py-4 bg-slate-900 text-white rounded-2xl font-black text-lg shadow-xl shadow-slate-900/20 hover:bg-slate-800 active:scale-95 transition-all disabled:bg-slate-300 disabled:shadow-none flex items-center justify-center gap-3">{isSubmitting ? <Loader2 className="animate-spin" /> : <Save size={24}/>} SIMPAN & KUNCI</button></div></div>
      ) : (
