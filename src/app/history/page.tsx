...
import { ArrowLeft, Package, Search, Loader2, X, MapPin, Calendar, UserCheck, AlertCircle, User, Tag, Clock, ImageIcon } from 'lucide-react'; 
...
export default function HistoryPage() {
  ...
  // [UBAH] State Filter Status disesuaikan dengan status DB ('selesai')
  const [activeTab, setActiveTab] = useState<"semua" | "dipinjam" | "pending_return" | "selesai">("semua");
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'selesai': return 'bg-green-100 text-green-600'; // [UBAH] 'kembali' jadi 'selesai'
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'selesai': return 'bg-green-50 text-green-500'; // [UBAH] 'kembali' jadi 'selesai'
  ...
  const getStatusLabel = (status: string) => {
    switch (status) {
      ...
      case 'selesai': return 'Selesai'; // [UBAH] 'kembali' jadi 'selesai'
      default: return status;
    }
  };

  const getDisplayImage = (loan: any) => {
    if (loan.return_proof_url) return { src: loan.return_proof_url, type: 'BUKTI PENGEMBALIAN' };
    if (loan.variants?.items?.base_image_url) return { src: loan.variants.items.base_image_url, type: 'FOTO BARANG' };
    return null;
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-20 font-sans">
      ...
      <header className="bg-white p-6 sticky top-0 z-20 shadow-sm border-b border-slate-100 space-y-4">
        ...
        {/* [UBAH] Filter Tabs: Mengubah label dan ID "Kembali" menjadi "Selesai" */}
        <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
          {[
            ...
            { id: "selesai", label: "Selesai", color: "bg-green-600", text: "text-white" } // Updated here
          ].map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`px-5 py-2 rounded-full text-[11px] font-black uppercase tracking-widest transition-all duration-300 whitespace-nowrap ${activeTab === tab.id ? 
              `${tab.color} ${tab.text} shadow-lg shadow-current/20 scale-105` : "bg-slate-100 text-slate-400 hover:bg-slate-200"}`}>{tab.label}</button>
          ))}
        </div>
      </header>
      ...
      <AnimatePresence>
        {selectedLoan && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/60 backdrop-blur-md p-4" onClick={() => setSelectedLoan(null)}>
            <motion.div layoutId={selectedLoan.id} className="bg-white w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
              
              <div className="relative h-64 bg-slate-100 group shrink-0">
                {(() => {
                  const displayImg = getDisplayImage(selectedLoan);
                  return displayImg ? (
                    <>
                      <img 
                        src={displayImg.src} 
                        ...
                        alt="Preview" 
                        onClick={() => setZoomedImage(displayImg.src)} 
                      />
                      <div className="absolute top-4 left-4">
                        <span className={`px-3 py-1 text-[10px] font-black rounded-lg backdrop-blur-md border ${
                          displayImg.type === 'BUKTI PENGEMBALIAN' 
                            ? 'bg-black/60 text-white border-white/20' 
                            : 'bg-white/80 text-slate-600 border-white/50'
                        }`}>
                          {displayImg.type}
                        </span>
                      </div>
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-slate-300">
                      <ImageIcon size={64} />
                      <p className="text-[10px] font-black uppercase mt-2 tracking-widest">Tidak Ada Foto</p>
                    </div>
                  );
                })()}

                <button onClick={() => setSelectedLoan(null)} className="absolute top-4 right-4 p-2 bg-black/20 backdrop-blur-md text-white rounded-full hover:bg-black/40 transition-colors"><X size={20} /></button>
              ...
              <div className="p-8 space-y-6 overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-slate-50 rounded-2xl">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Lokasi Rak</p>
                    <div className="flex items-center gap-1 text-slate-800">
                      <MapPin size={14} className="text-red-500" />
                      <p className="text-sm font-bold">{selectedLoan.variants?.location || "-"}</p>
                    </div>
                  </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-xs border-b border-slate-50 pb-3">
                    <span className="font-bold text-slate-400 uppercase tracking-widest">Waktu Pinjam</span>
                    <span className="font-black text-slate-800">{new Date(selectedLoan.borrow_date).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' })}</span>
                  </div>
                  
                  {/* [UBAH] Cek juga status 'selesai' */}
                  {(selectedLoan.status === 'kembali' || selectedLoan.status === 'selesai') && selectedLoan.return_date && (
                    <div className="flex items-center justify-between text-xs pt-1">
                      <span className="font-bold text-slate-400 uppercase tracking-widest">Waktu Kembali</span>
                      <span className="font-black text-green-600">{new Date(selectedLoan.return_date).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' })}</span>
                    </div>
                  )}

                  {selectedLoan.status === 'pending_return' && (
                    <div className="flex items-center gap-2 bg-amber-50 p-3 rounded-xl border border-amber-100 text-amber-700 text-xs font-bold mt-2">
      ...
      <AnimatePresence>
        {zoomedImage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4 backdrop-blur-md" onClick={() => setZoomedImage(null)}>
            <button onClick={() => setZoomedImage(null)} className="absolute top-6 right-6 p-2 bg-white/10 hover:bg-white/20 rounded-full text-white transition-colors z-50"><X size={32} /></button>
