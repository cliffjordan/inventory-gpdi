  ...
  const handleConfirmReturn = async () => {
    ...
    try {
      ...
      const { error: updateError } = await supabase.from('loans').update({
        ...
        reject_reason: null // Hapus alasan penolakan saat submit baru
      }).eq('id', selectedLoan.id);

      if (updateError) throw updateError;

      // 4. CATAT KE AUDIT LOG (CCTV)
                ...
                <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden bg-slate-50/50 border-t border-slate-50">
                  <div className="p-3 space-y-2">
                    {trans.loans.map((loan: any) => (
                      <div key={loan.id} className={`bg-white p-4 rounded-2xl border flex flex-col gap-3 ${loan.reject_reason ? 'border-red-500 bg-red-50/20' : 'border-slate-100'}`}>
                        <div className="flex items-center gap-4">
                          ...
                          {loan.status === 'pending_return' ? (
                            ...
                            <button onClick={() => setSelectedLoan(loan)} className={`w-10 h-10 text-white rounded-xl flex items-center justify-center shadow-lg transition-all ${loan.reject_reason ? 
                              'bg-red-500 animate-bounce' : 'bg-blue-600'}`}>
                              <Camera size={18}/>
                            </button>
                          )}
                        </div>
                        ...
                        {loan.reject_reason && (
                          <div className="bg-white p-3 rounded-xl border border-red-200 flex items-start gap-2">
                            ...
                            <div>
                              <p className="text-[10px] font-black text-red-600 uppercase">Ditolak / Perlu Revisi</p>
                              <p className="text-xs text-red-700 font-bold mt-1">{loan.reject_reason}</p>
                            </div>
                          </div>
                        )}
                      </div>
