...
import { 
  ...
  Package, BoxSelect, UserPlus, Calendar, Archive,
  // [FIX] Added missing icons below:
  Cpu, ClipboardList, Ticket, Stethoscope, History, Plus
} from 'lucide-react';
...
export default function SuperuserPage() {
  ...
  // --- STATE DATA ---
  const [activeView, setActiveView] = useState<'menu' | 'audit'>('menu');
  ...
  // --- STATE MODALS (ADVANCED TOOLS - JANITOR & GHOST) ---
  ...
  const [janitorMode, setJanitorMode] = useState<'menu' | 'items' | 'avatars' | 'returns'>('menu');
  const [janitorDateRange, setJanitorDateRange] = useState({ start: '', end: '' });
  const [isScanningStorage, setIsScanningStorage] = useState(false);
  const [scanStats, setScanStats] = useState<{ count: number, size: string, files: any[], rawData?: any[] } | null>(null);
  ...
  const handleLogout = async () => {
    const toastId = toast.loading("Menghapus sesi...", { id: 'logout-process' });
    try {
  ...
  const resetJanitor = () => {
    setJanitorMode('menu');
    setScanStats(null);
    setJanitorDateRange({ start: '', end: '' });
  };

  const scanOrphanedItems = async () => {
    setIsScanningStorage(true);
    try {
        const { data: files } = await supabase.storage.from('items').list();
        const { data: dbItems } = await supabase.from('items').select('base_image_url');
        const { data: dbVariants } = await supabase.from('variants').select('image_url');
        
        const usedUrls = new Set([
            ...(dbItems?.map(i => i.base_image_url) || []),
            ...(dbVariants?.map(v => v.image_url) || [])
        ]);

        const orphans = files?.filter(f => f.name !== '.emptyFolderPlaceholder' && !Array.from(usedUrls).some(url => url && url.includes(f.name))) || [];
        const totalSize = orphans.reduce((acc, f) => acc + (f.metadata?.size || 0), 0);
        setScanStats({
            count: orphans.length,
            size: (totalSize / 1024 / 1024).toFixed(2) + ' MB',
            files: orphans.map(f => f.name)
        });
    } catch (e) { toast.error("Scan Failed"); } finally { setIsScanningStorage(false); }
  };

  const scanOrphanedAvatars = async () => {
    setIsScanningStorage(true);
    try {
  ...
  const scanReturnHistory = async () => {
    if (!janitorDateRange.start || !janitorDateRange.end) return toast.error("Pilih rentang tanggal!");
    setIsScanningStorage(true);
    try {
        const { data: loans } = await supabase.from('loans')
            .select('*')
            .in('status', ['selesai', 'dikembalikan'])
            .gte('returned_at', janitorDateRange.start)
            .lte('returned_at', janitorDateRange.end + 'T23:59:59');

        if (!loans || loans.length === 0) {
            setScanStats({ count: 0, size: '0 MB', files: [], rawData: [] });
            return;
        }

        const filesToDelete: string[] = [];
        loans.forEach(l => {
            if (l.return_proof_url) {
                const parts = l.return_proof_url.split('/');
                filesToDelete.push(parts[parts.length - 1]);
            }
        });

        const estSize = (filesToDelete.length * 0.15).toFixed(2); 

        setScanStats({
            count: filesToDelete.length,
            size: `~${estSize} MB (Estimasi)`,
            files: filesToDelete,
            rawData: loans 
        });
  ...
  const executeJanitorCleanup = async () => {
    if (!scanStats || scanStats.count === 0) return;
    const toastId = toast.loading("Cleaning up...");
    try {
        let bucket = '';
        if (janitorMode === 'items') bucket = 'items';
        else if (janitorMode === 'avatars') bucket = 'avatars';
        else if (janitorMode === 'returns') bucket = 'inventory-proofs';

        if (janitorMode === 'returns' && scanStats.rawData) {
            const fileName = `BACKUP_LOANS_${janitorDateRange.start}_TO_${janitorDateRange.end}.json`;
            const jsonStr = JSON.stringify(scanStats.rawData, null, 2);
            ...
            const loanIds = scanStats.rawData.map(l => l.id);
            await supabase.from('loans').delete().in('id', loanIds);
            toast.success("Data Backup Downloaded & DB Rows Cleared", { id: toastId });
        }

        if (scanStats.files.length > 0) {
            const batchSize = 20;
            for (let i = 0; i < scanStats.files.length; i += batchSize) {
                const batch = scanStats.files.slice(i, i + batchSize);
                await supabase.storage.from(bucket).remove(batch);
            }
        }

        await logAction("Janitor Cleanup", `Cleaned ${scanStats.count} files from ${bucket}`);
        toast.success("Storage Cleaned Successfully!", { id: toastId });
        resetJanitor(); 
        setShowJanitorModal(false);
  ...
  const fetchApprovals = async () => {
    try {
        ...
        if (migrations) migrations.forEach(m => combinedData.push({ id: m.id, table: 'migration_requests', type: 'MIGRATION', user: (m.requested_by_profile as any)?.full_name || 'Unknown', request: 'Migrasi Akun (Merge)', details: `Source: ${m.source_id.slice(0,8)}... -> Target: ${m.target_id.slice(0,8)}...`, raw: m, time: new Date(m.created_at).toLocaleDateString() }));
        ...
        if (attendanceReqs) attendanceReqs.forEach(a => combinedData.push({ id: a.id, table: 'attendance_change_requests', type: 'ATTENDANCE', user: (a.requested_by_profile as any)?.full_name || 'Unknown', request: `Ubah Absensi: ${a.old_status} -> ${a.new_status}`, details: `Alasan: ${a.reason}`, raw: a, time: new Date(a.created_at).toLocaleDateString() }));
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-sans text-slate-100 pb-24 relative overflow-hidden">
        ...
        {activeView === 'menu' && (
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <MenuButton onClick={() => { resetJanitor(); setShowJanitorModal(true); }} icon={<HardDrive size={32} className="text-cyan-500"/>} label="Storage Janitor" />
      ...
      <AnimatePresence>
        ...
        {/* NEW JANITOR MODAL (UPDATED) */}
        {showJanitorModal && (
            <Modal title="Storage Janitor" onClose={() => setShowJanitorModal(false)}>
                {/* 1. MENU UTAMA JANITOR */}
                {janitorMode === 'menu' && (
                    <div className="space-y-3">
                        <p className="text-sm text-slate-400 mb-4 text-center">Pilih area yang ingin dibersihkan:</p>
                        <button onClick={() => { setJanitorMode('items'); scanOrphanedItems(); }} className="w-full p-4 bg-slate-800 hover:bg-slate-700 rounded-xl flex items-center gap-4 border border-slate-700 transition">
                            <div className="w-12 h-12 bg-cyan-900/30 rounded-full flex items-center justify-center text-cyan-400"><Package size={24}/></div>
                            <div className="text-left"><h4 className="font-bold text-white">Clean Items History</h4><p className="text-xs text-slate-500">Hapus foto barang tak terpakai</p></div>
                        </button>
                        <button onClick={() => { setJanitorMode('avatars'); scanOrphanedAvatars(); }} className="w-full p-4 bg-slate-800 hover:bg-slate-700 rounded-xl flex items-center gap-4 border border-slate-700 transition">
                            <div className="w-12 h-12 bg-pink-900/30 rounded-full flex items-center justify-center text-pink-400"><Users size={24}/></div>
                            <div className="text-left"><h4 className="font-bold text-white">Clean Avatars</h4><p className="text-xs text-slate-500">Hapus foto profil lama</p></div>
                        </button>
                        <button onClick={() => setJanitorMode('returns')} className="w-full p-4 bg-slate-800 hover:bg-slate-700 rounded-xl flex items-center gap-4 border border-slate-700 transition">
                            <div className="w-12 h-12 bg-amber-900/30 rounded-full flex items-center justify-center text-amber-400"><Archive size={24}/></div>
                            <div className="text-left"><h4 className="font-bold text-white">Clean Return History</h4><p className="text-xs text-slate-500">Backup & hapus data pengembalian lama</p></div>
                        </button>
                    </div>
                )}

                {/* 2. MODE PILIH TANGGAL (KHUSUS RETURNS) */}
                {janitorMode === 'returns' && !scanStats && (
                    <div className="space-y-4">
                        <button onClick={() => setJanitorMode('menu')} className="text-xs text-slate-500 flex items-center gap-1 hover:text-white mb-2"><ArrowLeft size={12}/> Back to Menu</button>
                        <div className="p-4 bg-amber-900/10 border border-amber-900/30 rounded-xl">
                            <p className="text-xs text-amber-500 font-bold mb-2 flex items-center gap-2"><AlertTriangle size={14}/> WARNING</p>
                            <p className="text-xs text-slate-400">Data dan foto bukti pengembalian dalam rentang tanggal ini akan <b>DIHAPUS PERMANEN</b> dari database.</p>
                            <p className="text-xs text-slate-400 mt-2">Sistem akan melakukan <b>Auto-Backup (Download JSON)</b> sebelum penghapusan.</p>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                            <div><label className="text-[10px] uppercase text-slate-500 font-bold">Start Date</label><input type="date" className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2 text-white text-sm mt-1" onChange={(e) => setJanitorDateRange({...janitorDateRange, start: e.target.value})}/></div>
                            <div><label className="text-[10px] uppercase text-slate-500 font-bold">End Date</label><input type="date" className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2 text-white text-sm mt-1" onChange={(e) => setJanitorDateRange({...janitorDateRange, end: e.target.value})}/></div>
                        </div>
                        <button onClick={scanReturnHistory} className="w-full py-3 bg-amber-600 hover:bg-amber-500 text-white font-bold rounded-xl mt-4">Scan Data</button>
                    </div>
                )}

                {/* 3. HASIL SCAN & EKSEKUSI */}
                {scanStats && (
                    <div className="space-y-4">
                        <button onClick={resetJanitor} className="text-xs text-slate-500 flex items-center gap-1 hover:text-white mb-2"><ArrowLeft size={12}/> Cancel</button>
                        
                        {isScanningStorage ? (
                            <div className="text-center py-10"><Loader2 className="animate-spin mx-auto text-cyan-500 mb-2" size={32}/><p className="text-xs font-mono text-cyan-400">Scanning Storage...</p></div>
                        ) : (
                            <div className="text-center">
                                <div className="w-20 h-20 mx-auto bg-slate-800 rounded-full flex items-center justify-center mb-4 border border-slate-700">
                                    <Trash2 size={32} className="text-red-500"/>
                                </div>
                                <h3 className="text-2xl font-black text-white">{scanStats.count} <span className="text-base font-normal text-slate-400">Files Found</span></h3>
                                <p className="text-sm font-mono text-cyan-400 mt-1">Potential Savings: {scanStats.size}</p>
                                
                                <div className="bg-slate-800 rounded-xl p-3 mt-6 text-left max-h-32 overflow-y-auto border border-slate-700">
                                    <p className="text-[10px] text-slate-500 uppercase font-bold mb-2 sticky top-0 bg-slate-800">Preview Files:</p>
                                    {scanStats.files.map((f, i) => (
                                        <p key={i} className="text-xs text-slate-300 truncate font-mono">{f}</p>
                                    ))}
                                </div>

                                <button 
                                    onClick={executeJanitorCleanup} 
                                    disabled={scanStats.count === 0}
                                    className="w-full py-4 bg-red-600 hover:bg-red-500 text-white font-bold rounded-xl mt-6 flex items-center justify-center gap-2 shadow-lg shadow-red-900/20"
                                >
                                    {janitorMode === 'returns' ? <><Download size={18}/> Backup & Delete Forever</> : <><Trash2 size={18}/> Delete Forever</>}
                                </button>
                            </div>
                        )}
                    </div>
